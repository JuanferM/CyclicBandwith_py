...
cons1 = 1 # Default value of the constraint x[0] == cons1
...
del(drule, orule, krule, srule, wrrule, cons1r)
...
# On rajoute maintenant la 2e et 3e clauses
for i in range(n):
    c2 = [] # Deuxième "grande" clause
    for j in range(n):
        c3 = [-(n*i+j)-1] # 3e grande clause
        for k in range(n):
            if k != j:
                c3.append(-(n*k+i)-1)
        c2.append(n*i+j+1)
...
# On rajoute la 4e et dernière clause
for (u, v) in edges:
    for i in range(n):
        for j in range(n):
            if P[i][j]:
